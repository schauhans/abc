...
function touchStarted() {
  if(mapInit){
    ...
    if(millis()-lastTouch < doubleTouchInterval && millis()-lastPoint > 1000){
      ...
      // send to server instead of just pushing to our own array
...
let locations = [];
//add locations on launch of all current locations
socket.on("current-locations", function(data){
  locations.push(...data);
})
...
socket.on("send-all-locations", function(data){
  locations.push(data)
  console.log("Locations on the client array:", locations);
})
