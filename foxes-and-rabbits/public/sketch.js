...
// ── SET YOUR GAME LOCATION ─────────────────────────────────
// Must match MAP_CENTER in server.js exactly.
// Go to maps.google.com, right-click your location → copy coordinates.
const MAP_CENTER_LAT = 31.15270;
const MAP_CENTER_LNG = 121.47800;
...
function draw() {
    ...
    visibleTraps.forEach(t => {
        ...
        circle(0, 0, metersToPixel(6, currentLatitude || MAP_CENTER_LAT) * 2);
        drawingContext.setLineDash([]);
        pop();
    });

    // ── draw carrots (rabbit only) ────────────────────────
    // Orange circle with a small stem — only sent to rabbits by server
