...
io.on('connection', (socket) => {
    ...
    socket.on("my-role", function(data){
        console.log("my role is", data.role);
        // if frog:
        ...
        if(data.role == "frog"){
            ...
            frogs.push(frogData);
            console.log(frogs);
            if(conductor){//conductor logged in -> new frog add
                io.to(conductor).emit("new-frog", frogData);
            }
        // if conductor:
        //     store conductor socket id to conductor global variable
        //     tell conductor which frogs are currently online
        } else{ //conductor
            conductor = socket.id;
    ...
    socket.on("trigger-frog", function(socketID){
        console.log("triggering frog with socket id", socketID);
        io.to(socketID).emit("play-frog");
    });
    ...
    socket.on("disconnect", function(){
        ...
        if(idx != -1){
            // delete frog
            frogs.splice(idx, 1); //splice modifies the original array, 1st arg is index, 2nd is how many elements to delete
            console.log("a frog disconnected: ", frogs);
        } 
        
        if(socket.id == conductor){//the conductor is disconnecting
            conductor = undefined;
        }
        
        if(conductor){
            io.to(conductor).emit("frog-disconnected", socket.id);
        }
